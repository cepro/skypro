    ...
    def run(self) -> pd.DataFrame:
        ...
        while current_start_t < end_t:
            ...
            block_df_in = add_total_vol_rates_to_df(
                ...
                live_or_final_tag="_final"
            )

            logging.info(f"Optimising range {block_df_in.index[0]} -> {block_df_in.index[-1]}...")
            block_df_out, block_num_nan = run_one_optimisation(
                df_in=block_df_in,
                init_soe=init_soe,
...
def run_one_optimisation(
    ...
    df_in needs the following columns:
    - solar_not_supplying_load
    - load_not_supplied_by_solar
    - max_charge_from_grid
    - max_discharge_to_grid
    - min_charge
    - min_discharge
    - bess_max_charge
    - bess_max_discharge
    - mkt_vol_rate_grid_to_batt (or mkt_vol_rate_final_grid_to_batt)
    - int_vol_rate_solar_to_batt (or int_vol_rate_final_solar_to_batt)
    - mkt_vol_rate_batt_to_grid (or mkt_vol_rate_final_batt_to_grid)
    - int_vol_rate_batt_to_load (or int_vol_rate_final_batt_to_load)
    - time_into_sp
    """
    # Detect column naming convention (with or without _final_ tag)
    if "mkt_vol_rate_final_grid_to_batt" in df_in.columns:
        rate_prefix_mkt = "mkt_vol_rate_final_"
        rate_prefix_int = "int_vol_rate_final_"
    else:
        rate_prefix_mkt = "mkt_vol_rate_"
        rate_prefix_int = "int_vol_rate_"
    ...
    for timeslot_idx in timeslots:
        ...
        lp_var_bess_soe.append(
            pulp.LpVariable(
                ...
                lowBound=0.0,
                upBound=bess_config.energy_capacity
            )
        )
        ...
        mkt_rate_grid_to_batt = df_in.iloc[timeslot_idx][f"{rate_prefix_mkt}grid_to_batt"]
        int_rate_solar_to_batt = df_in.iloc[timeslot_idx][f"{rate_prefix_int}solar_to_batt"]
        mkt_rate_batt_to_grid = df_in.iloc[timeslot_idx][f"{rate_prefix_mkt}batt_to_grid"]
        int_rate_batt_to_load = df_in.iloc[timeslot_idx][f"{rate_prefix_int}batt_to_load"]
        if np.any(np.isnan([
            mkt_rate_grid_to_batt,
            int_rate_solar_to_batt,
            mkt_rate_batt_to_grid,
            int_rate_batt_to_load
        ])):
            # the costs function throws an exception when these are NaN, so set to zero but disallow any activity
            # by adding constraints
            mkt_rate_grid_to_batt = 0
            int_rate_solar_to_batt = 0
            mkt_rate_batt_to_grid = 0
            int_rate_batt_to_load = 0
        ...
        lp_costs.append(
            lp_var_bess_charges_from_grid[timeslot_idx] * mkt_rate_grid_to_batt +
            lp_var_bess_charges_from_solar[timeslot_idx] * int_rate_solar_to_batt +
            lp_var_bess_discharges_to_grid[timeslot_idx] * mkt_rate_batt_to_grid +
            lp_var_bess_discharges_to_load[timeslot_idx] * int_rate_batt_to_load
        )
    ...
    if block_config.no_optional_charging_in_lowest_priced_quantile is not None:
        for _, df_day in df_in.groupby(df_in.index.date):
            df_lowest = _get_lowest_valued_rows(block_config.no_optional_charging_in_lowest_priced_quantile, df_day, f"{rate_prefix_mkt}grid_to_batt")
    ...
    if block_config.max_avg_cycles_per_day:
        ...
        problem += pulp.lpSum(lp_var_bess_discharges) <= (block_config.max_avg_cycles_per_day * days_in_block * bess_config.energy_capacity)
    ...
    for timeslot_idx in timeslots[1:]:
        problem += (
            ...
            + lp_var_bess_charges_from_solar[timeslot_idx - 1] * bess_config.charge_efficiency
            + lp_var_bess_charges_from_grid[timeslot_idx - 1] * bess_config.charge_efficiency
    ...
    _ensure_merit_order_of_charge_and_discharge(df_in, df_sol)
    ...
    df_ret["bess_losses"] = (
        (df_sol["solar_to_batt"] + df_sol["grid_to_batt"]) * (1 - bess_config.charge_efficiency)
    )
...
def _ensure_merit_order_of_charge_and_discharge(df_in: pd.DataFrame, df_sol: pd.DataFrame) -> None:
    """
    ...
    check = (
        (when_charging_from_grid["solar_to_batt"] - df_in["solar_not_supplying_load"])
        > tolerance
    ).sum()
    ...
    check = (
            (when_discharging_to_grid["batt_to_load"] - df_in["load_not_supplied_by_solar"])
            > tolerance
    ).sum()
